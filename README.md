...
主语言：Python 3.11+  
包管理：`uv` 或 `pip`，任选一个  
模型接口：优先 OpenAI Responses API / Agents SDK；如果没有 API key，可以先用 mock model 完成 loop 结构  
日志格式：JSONL  
文档格式：Markdown  
MCP：Python 或 TypeScript 均可，计划默认 Python 优先  
...
创建 `mini_agent/loop.py`，先不用真实模型，写一个 `MockModel`，让它按预设脚本返回：

1. 第 1 轮要求调用 calculator。
2. 第 2 轮根据 calculator 结果输出 final answer。

创建 `mini_agent/tools.py`：
...
2. 写 `run_agent(task, max_turns=5)`。
...
扩展 `mini_agent/tools.py`，实现 3 个工具：

- `calculator(expression)`
- `read_json(path)`
- `search_notes(query)`

创建 `mini_agent/tool_registry.py`：
...
创建 `mini_agent/planning_modes.py`，实现 3 种 mock 策略：
...
4. 写一个 `scripts` 或简单命令，用 `python -m json.tool` 检查 JSON 是否可读。
...
创建 `cli_harness/cli.py`：

- 支持 `python -m cli_harness.cli "your task"`。
- 支持 `--session-id`。
- 支持 `--workspace`。
- 调用前一周的 `mini_agent`。
...
创建 `cli_harness/filesystem_tools.py`：

- `list_files(path=".")`
- `read_file(path, max_bytes=20000)`
...
- `cli_harness/filesystem_tools.py`
...
创建 `cli_harness/shell_tool.py`：

- `run_shell(command, timeout=30)`
- 命令白名单：`ls`、`pwd`、`python`、`pytest`、`rg`、`sed`。
...
创建 `cli_harness/checkpoint.py`：

- `create_checkpoint(workspace, reason)`
- `list_checkpoints()`
...
```text
修复 sample_project 的测试失败，只允许修改 calculator.py，修复后运行测试。
```
...
- `search_memory(query: str)`：搜索 `memory/MEMORY.md`。
- `save_memory(text: str, tags: list[str])`：追加 memory。
- `list_tasks(status: str = "open")`：读取 `memory/tasks.json`。
...
- 更新 `mini_agent/tool_registry.py`
...
创建 `mini_agent/multi_agent_demo.py`。
...
- `mini_agent/multi_agent_demo.py`
